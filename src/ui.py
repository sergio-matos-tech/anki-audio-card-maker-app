...
class AnkiAlignerApp(ctx.CTk):
    ...
    def _build_ui_layout(self):
        ...
        self.profile_label = ctx.CTkLabel(self.config_frame, text="Anki Profile Name:")
        self.profile_label.grid(row=0, column=0, padx=15, pady=15, sticky="w")
        
        self.profile_entry = ctx.CTkEntry(self.config_frame, textvariable=self.profile_name, width=150)
        ...
        self.console_box = ctx.CTkTextbox(self, height=160, font=ctx.CTkFont(family="Consolas", size=11))
        self.console_box.pack(pady=15, padx=30, fill="x")
        self.console_box.configure(state="disabled")
        
        self.status_banner = ctx.CTkLabel(
            self, text="", height=45, corner_radius=6,
            font=ctx.CTkFont(size=12, weight="bold"), text_color="white"
        )
        
        self.execute_btn = ctx.CTkButton(
            self, text="⚡ GENERATE & SYNC ASSETS", 
            command=self._trigger_pipeline_execution, 
            height=40, font=ctx.CTkFont(size=14, weight="bold"),
    ...
    def _handle_browse_directory(self):
        ...
        if directory:
            ...
            self._write_to_console(f"📁 [Directory] Source folder locked: {directory}")
    ...
    def _trigger_pipeline_execution(self):
        ...
        if target_dir in ["No folder selected...", ""]:
            self._write_to_console("[Error] Please select a valid source folder first.")
            return
        ...
        save_path = filedialog.asksaveasfilename(
            title="Choose where to save the Anki import file",
        ...
        if not save_path:
            self._write_to_console("⚠️ [Warning] Save operation canceled by user.")
            return
    ...
    def _finalize_ui_state(self, success: bool):
        ...
        if success:
            self.status_banner.configure(
                text="🎉 SUCCESS: Import file generated and audio assets synced to Anki!\n"
                     "👉 NEXT STEP: Open Anki, click 'Import File' and select your compiled .txt file.",
                fg_color="#10b981"
            )
        else:
            self.status_banner.configure(
                text="❌ PIPELINE ERROR: Synchronization failed. Check the console logs above.",
                fg_color="#ef4444"
            )
